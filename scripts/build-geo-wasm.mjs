...
const GO_MOD_DIR = path.join(REPO_ROOT, "geo-wasm");
const OUT_DIR = path.join(REPO_ROOT, "meshchatx", "src", "frontend", "public", "vendor", "geo-wasm");
const WASM_NAME = "geo.wasm";
...
function findWasmExec() {
    const fromEnv = process.env.GEO_GO_WASM_EXEC;
...
function main() {
    if (process.env.GEO_WASM_SKIP === "1") {
        console.log("build-geo-wasm: GEO_WASM_SKIP=1, skipping.");
        process.exit(0);
    }

    const requireWasm =
        process.env.MESHCHATX_REQUIRE_GEO_WASM === "1" || process.env.MESHCHATX_REQUIRE_GEO_WASM === "true";

    if (!fs.existsSync(path.join(GO_MOD_DIR, "go.mod"))) {
        const msg = "build-geo-wasm: geo-wasm/go.mod missing, skipping.";
        if (requireWasm) {
            console.error(msg);
            process.exit(1);
        }
    ...
    if (goCheck.status !== 0) {
        if (process.env.MESHCHATX_OFFLINE_BUILD === "1") {
            ...
            if (fs.existsSync(wasmPath) && fs.existsSync(execPath)) {
                console.log("build-geo-wasm: go missing but artifacts present (offline).");
                process.exit(0);
            }
            console.error("build-geo-wasm: MESHCHATX_OFFLINE_BUILD=1 but artifacts missing and go unavailable.");
            process.exit(1);
        }
        const msg = "build-geo-wasm: go not found, skipping (WGS84-only fallback will be used).";
        if (requireWasm) {
            console.error(msg);
            process.exit(1);
        }
    ...
    if (!execSrc) {
        console.error("build-geo-wasm: wasm_exec.js not found under GOROOT");
        process.exit(1);
    }
    ...
    console.log(`build-geo-wasm: OK (${wasmBuf.length} bytes WASM, SRI written to vendor/geo-wasm/)`);
}
