...
<div class="relative" bind:this={dropdownEl}>
    ...
    {#if showPublishMenu}
        <div class="absolute right-0 top-full mt-1 w-72 z-50">
            <div
                class="dropdown-caret pointer-events-none absolute -top-[4px] right-3 border-t border-l border-sem-border"
                aria-hidden="true"
            ></div>
            <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-xl border border-sem-border py-2">
                ...
                {:else}
                    {#each pageNodes as pn (pn.node_id)}
                        ...
                        >
                            <div
                                class="w-2 h-2 rounded-full shrink-0 {pn.running ? 'bg-green-500' : 'bg-gray-400'}"
                            ></div>
