        ...
        {#if rendererChip && !isLoadingNodePage}
            <div class="relative shrink-0 hidden sm:inline-flex">
                ...
                {#if rendererPopoverOpen}
                    <div class="absolute left-0 top-full mt-1 z-60 w-64">
                        <div
                            class="dropdown-caret pointer-events-none absolute -top-[4px] left-3 border-t border-l border-sem-border"
                            aria-hidden="true"
                        ></div>
                        <div
                            class="rounded-lg border border-sem-border bg-sem-surface px-3 py-2 text-xs leading-snug text-sem-fg shadow-lg"
                        >
    ...
    {#if !isPrivate && (pageArchives.length > 0 || nodePageContent)}
        <div class="my-auto shrink-0 relative">
            ...
            {#if isArchiveDropdownOpen}
                <div class="absolute right-0 mt-2 w-64 z-50">
                    <div
                        class="dropdown-caret pointer-events-none absolute -top-[4px] right-3 border-t border-l border-sem-border"
                        aria-hidden="true"
                    ></div>
                    <div
                        class="bg-sem-surface border border-sem-border rounded-lg shadow-lg overflow-hidden text-sem-fg"
                    >
                        <div
