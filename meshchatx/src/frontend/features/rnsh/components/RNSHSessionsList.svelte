...
<script lang="ts">
    ...
    import type { RnshSession } from "../lib/types.js";

    interface Props {
        sessions: RnshSession[];
    ...
    function statusLabel(session: RnshSession): string {
        return t(`rnsh.status_${session.status}`);
    }

    function subtitle(session: RnshSession): string {
        return getSessionSubtitle(session, t("rnsh.listen_mode"));
    }
</script>
