...
export const MESHCHAT_THEME_VARIABLES_LIGHT: Record<string, string> = {
    "--mc-canvas": "#f8fafc",
    "--mc-surface": "#ffffff",
    "--mc-border": "#e5e7eb",
...
export const MESHCHAT_THEME_VARIABLES_DARK: Record<string, string> = {
    "--mc-canvas": "#09090b",
    "--mc-surface": "#09090b",
    "--mc-border": "#27272a",
...
export function tailwindSemanticColorExtend(): Record<string, string> {
    return {
...
function serializeVarsBlock(vars: Record<string, string>): string {
    return Object.entries(vars)
        .map(([k, v]) => `  ${k}: ${v};`)
        .join("\n");
}

export function injectMeshchatThemeVariables(
