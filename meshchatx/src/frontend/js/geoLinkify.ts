 ...
 */
export function decodeMaidenhead(locator: unknown): { lat: number; lon: number } | null {
...
function isMaidenhead(token: unknown): boolean {
    return decodeMaidenhead(token) !== null;
}

function geoAnchor(display: string, text: string): string {
    return (
