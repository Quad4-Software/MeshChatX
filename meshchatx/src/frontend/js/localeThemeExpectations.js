 ...
 */
export function expectedBootTheme(storedTheme, prefersDark = false) {
 ...
 */
export function expectedVisualiserClearColor(dark) {
    if (dark) {
 ...
 * @returns {boolean}
 */
export function expectedVisualiserIsDark(configTheme, htmlHasDarkClass) {
 ...
 * @returns {string}
 */
export function expectedUiLocalePack(raw) {
    const code = normalizeUiLocaleCode(raw);
    const packs = listLocaleCodes();
    if (!packs.includes(code)) {
        throw new Error(`reference violation: normalized locale ${code} not in packs`);
    }
    return code;
}
