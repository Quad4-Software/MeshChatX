...
class DownloadUtils {
    ...
    static async downloadFromApiResponse(response, defaultFilename, options) {
    ...
    static downloadFromBase64(filename, fileBytesBase64, options) {
    ...
    static async downloadFile(filename, blob, options) {
        const safeName = DownloadUtils.sanitizeDownloadFilename(filename, "download");
        if (isAndroidSaveBridge()) {
