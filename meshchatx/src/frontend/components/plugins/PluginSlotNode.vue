...
<template>
    ...
    <div v-else-if="node.type === 'input'" class="space-y-1.5">
        ...
        <textarea
            v-if="node.multiline"
            class="w-full rounded-lg border border-gray-300 dark:border-zinc-700 bg-sem-surface px-3 py-2.5 text-sm text-gray-900 dark:text-gray-100 placeholder:text-gray-400 focus:outline-hidden focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 min-h-[6rem]"
        ...
        <input
            v-else
            class="w-full rounded-lg border border-gray-300 dark:border-zinc-700 bg-sem-surface px-3 py-2.5 text-sm text-gray-900 dark:text-gray-100 placeholder:text-gray-400 focus:outline-hidden focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500"
            type="text"
            :placeholder="node.placeholder || ''"
            :value="node.value || ''"
            @input="$emit('input', { id: node.id, value: $event.target.value })"
    ...
    <div
        ...
        class="rounded-xl border border-sem-border bg-gray-50/70 dark:bg-zinc-900/40 p-4 sm:p-5 space-y-4"
    >
    ...
    <div v-else-if="node.type === 'list'" class="space-y-2">
        <div
            ...
            class="rounded-lg border border-sem-border overflow-hidden divide-y divide-gray-200 dark:divide-zinc-800 bg-sem-surface"
        >
            <PluginSlotNode
        ...
        <p
            ...
            class="rounded-lg border border-dashed border-gray-300 dark:border-zinc-700 px-4 py-8 text-center text-sm text-sem-fg-muted"
        >
            {{ node.emptyText || "" }}
        </p>
    </div>
...
export default {
    ...
    computed: {
        ...
        badgeClass() {
            ...
            const map = {
                ...
                muted: "bg-gray-100 text-gray-700 dark:bg-zinc-800 text-sem-fg-muted",
            };
    ...
    methods: {
        actionButtonClass(action) {
            const base =
                "inline-flex items-center justify-center px-4 py-2 rounded-lg text-sm font-medium transition-colors focus:outline-hidden focus:ring-2 focus:ring-blue-500/40 w-fit";
            if (action.variant === "secondary") {
                return `${base} border border-gray-300 dark:border-zinc-600 bg-sem-surface text-gray-700 dark:text-gray-200 hover:bg-sem-surface-muted`;
            }
            if (action.variant === "danger") {
                return `${base} border border-red-300 dark:border-red-800 bg-sem-surface text-red-600 dark:text-red-300 hover:bg-red-50 dark:hover:bg-red-950/30`;
            }
