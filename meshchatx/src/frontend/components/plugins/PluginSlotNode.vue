...
<template>
    ...
    <div v-else-if="node.type === 'input'" class="space-y-1.5">
        ...
        <textarea
            v-if="node.multiline"
            class="w-full rounded-lg border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-3 py-2.5 text-sm text-gray-900 dark:text-gray-100 placeholder:text-gray-400 focus:outline-hidden focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 min-h-[6rem]"
        ...
        <input
            v-else
            class="w-full rounded-lg border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-3 py-2.5 text-sm text-gray-900 dark:text-gray-100 placeholder:text-gray-400 focus:outline-hidden focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500"
            type="text"
            :placeholder="node.placeholder || ''"
            :value="node.value || ''"
            @input="$emit('input', { id: node.id, value: $event.target.value })"
