...
def _normalize_host(host: str | None) -> str:
    ...
    if host is None:
        return ""
    host = str(host).strip()
    if host == "" or host in {"*", "0.0.0.0", "::", "[::]"}:  # nosec: BAN-B104
        return ""
    return host
...
def is_port_in_use(host: str | None, port, *, kind: str = "tcp") -> bool:
    ...
    if normalized == "":
        candidates.append((socket.AF_INET, "0.0.0.0"))  # nosec: BAN-B104
...
def describe_port_conflict(
    ...
    *,
    ...
    coerced_port = _coerce_port(port)
    host_label = _normalize_host(host) or "0.0.0.0"  # nosec: BAN-B104
