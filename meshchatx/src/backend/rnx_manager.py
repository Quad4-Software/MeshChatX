...
"""RNX session manager for MeshChatX.

Tracks multiple rnx subprocess sessions for remote command execution and
listeners, keeps them running while the app is active, and persists session
definitions plus output history.
"""
...
# Matches the listener address that rnx logs on startup, e.g.
# "rnx listening for commands on <a1b2c3...>" or "Listening on : <...>".
...
class RNXSession:
    """Runtime state for a single rnx session."""
    ...
    @staticmethod
    def _rnx_module_available():
        try:
            return importlib.util.find_spec(_RNX_MODULE) is not None
    ...
    @staticmethod
    def _resolve_rnx_launcher():
        """Return argv prefix for launching rnx.
        ...
        if RNXSession._rnx_module_available():
            ...
            return [sys.executable, "-m", _RNX_MODULE]
        executable = shutil.which("rnx")
    ...
    def _build_command(self):
        command = list(self._resolve_rnx_launcher())

        # Attach rnx to the same Reticulum config directory (and therefore the
        # same shared instance and rpc_key) as the MeshChatX app. Without this
        # rnx bootstraps its own default config, becomes a mismatched shared
        ...
        if bool(self.config.get("detailed")):
            command.append("-d")
        ...
        result_timeout = self.config.get("result_timeout")
        if result_timeout not in (None, ""):
            command.extend(["-W", str(result_timeout)])

        stdin_value = self.config.get("stdin")
        if isinstance(stdin_value, str) and stdin_value:
            command.extend(["--stdin", stdin_value])

        stdout_limit = self.config.get("stdout_limit")
        if stdout_limit not in (None, ""):
            command.extend(["--stdout", str(stdout_limit)])

        stderr_limit = self.config.get("stderr_limit")
        if stderr_limit not in (None, ""):
            command.extend(["--stderr", str(stderr_limit)])
        ...
        if mode == "listen":
            ...
            if bool(self.config.get("no_announce")):
                command.append("-b")
            ...
            destination = self.destination
            if not destination:
                raise ValueError("Destination hash is required for execute mode")
            ...
            if mode == "interactive":
                ...
                if not remote_command:
                    raise ValueError("Command is required for execute mode")
        ...
        with self._lock:
            ...
            if self._stop_requested:
                ...
                if exit_code != 0 and not self.last_error:
                    self.last_error = f"rnx exited with code {exit_code}"
            self.updated_at = time.time()
        self.manager._on_session_change(self)
        self.manager.save()
...
class RNXManager:
    """Manage multiple rnx sessions and persisted state."""
