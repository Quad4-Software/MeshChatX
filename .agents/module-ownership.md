...
| Auth / CSRF       | csrf.py, app_security_settings.py, ip_allowlist.py             | auth/                   | http/ws/dispatch.py          | test_csrf*, test_http_auth_security.py, test_app_security_features.py, test_sanitizer_auth_ws_oracles.py, test_access_attempts_*         | Auth gate in features/app-shell/                                                              |
...
| Database / backup | database/_, recovery/_                                         | database/, maintenance/ | (broadcasts on restore)      | backup/restore skill tests                                                                                                               | About / maintenance settings                                                                  |
...
in routes/**init**.py. Route modules import shared names directly or through
per-domain _names.py shims over http/meshchat_names.py; free names resolve through
live_names.inject_meshchat_names where the shim layer is used. Wire constants live
in backend/constants.py and the typed app surface in http/context.py.
