---
name: database-migrations-backups
description: SQLite schema bumps, backups, snapshots, and restore safety. Use when changing migrations or backup/restore paths. Do not conflate identity-key restore with zip restore.
---
...
Private key only vs full history/settings/identity tree restore are different operations with different APIs. Full table: .agents/overview.md (Identity key restore vs database restore section).
