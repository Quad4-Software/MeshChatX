---
name: deferred-network-startup
description: HTTP-up vs RNS-ready status, 503s for retryable init, Electron loading probes, Android RNS panic containment. Use when changing boot order or early API behaviour.
---
...
- App boot uses startup interpreters that can mount recovery UI when failed still allows degraded UI.
