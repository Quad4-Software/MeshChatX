...
description: Extract HTTP routes and WS handlers under backend/http without behaviour change. Use when moving handlers among routes/<domain>.py or ws/handlers_*.py.
---
...
- Splitting or moving handlers among `routes/<domain>.py` modules
...
8. Domain splits start from existing `routes/<domain>.py` modules or residual shared helpers.
   Do not re-extract routes from `meshchat.py`.
...
Tests that need to patch a symbol patch it on the consuming module, not on
`meshchatx.meshchat`, unless the consumer still resolves through meshchat lazily.
...
their tests retargeted to patch the consuming module instead.
