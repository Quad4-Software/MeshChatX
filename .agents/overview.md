...
Always-on editor rule: .agents/conventions/reticulum-zen.md.
...
Linux also uses a seccomp BPF denylist in seccomp_sandbox.py after Landlock is applied. On Windows the equivalent layer is AppContainer via appcontainer_sandbox.py and appcontainer_launcher.py. See .agents/skills/landlock-sqlite/SKILL.md for the kernel docs and the layered model.
