...
- **App shortcuts** (long-press the launcher icon): open **Messages** (meshchatx://app/messages) and **Call** (meshchatx://app/call). The WebView handles these in the Svelte app shell via handleProtocolLink (features/app-shell/lib/appShellLinks.ts). Message notification taps can open a specific conversation with meshchatx://app/messages/<destination_hash>.
