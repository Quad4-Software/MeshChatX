...
"""References for map drawing ownership (identity_hash column)."""
...
@pytest.fixture
def db(tmp_path):
    path = str(tmp_path / "map_drawings_expected.db")
...
def test_delete_drawing_scoped_to_identity_hash(db):
    owner_a = "aa" * 16
    owner_b = "bb" * 16
...
def test_update_drawing_scoped_to_identity_hash(db):
    owner_a = "cc" * 16
    owner_b = "dd" * 16
    now = datetime.now(UTC)
    db.provider.execute(
