...
"""References and adversarial cases for local (self) LXMF messaging.
...
def test_is_self_lxmf_param(local_self_app, dest, expected):
    app = local_self_app
    assert app._is_self_lxmf_destination(dest) is expected


def test_is_self_lxmf_identity_hash(local_self_app):
    app = local_self_app
...
async def test_local_self_sqlite_row_delivered_local(local_self_app):
    ...
    with (
        ...
        await app.send_message(destination_hash=LOCAL_LXMF, content="reference persist")
...
def test_rejects_if_implementation_always_returns_true(local_self_app):
    """Guard: mutating reference expectation must fail (documents test strength)."""
    app = local_self_app
