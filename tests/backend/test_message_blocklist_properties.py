...
"""Independent references for message blocklist match, import, and inbound banish."""
...
from tests.backend.lxmf_tools_support import (
    ...
    PEER_FRIEND,
    PEER_SPAMMER,
    expected_blocklist_first_match,
    make_inbound_lxmf,
    peer_blocked,
...
def test_default_scope_is_non_contacts():
...
def test_forces_a_match_target_on():
...
def test_blocklist_scope_matrix(scope, is_contact, expect):
...
def test_first_match_equals_independent_model(texts, is_contact, hay):
    blocklist = normalize_message_blocklist(
        {
            "scope": "everyone",
            "match_message": True,
    ...
    expected = expected_blocklist_first_match(
        blocklist,
        "peer",
        is_contact=is_contact,
        message_haystack=hay,
