...
"""References for inbound LXMF stranger attachment policy.
...
def expected_stranger_inbound_outcome(
    *,
    block_all: bool,
...
def _attachment_field_sets():
    return {
        "file": {
            LXMF.FIELD_FILE_ATTACHMENTS: [[b"reference.txt", _ORACLE_SECRET]],
        },
        "image": {
...
def _deliver(app, source, fields, content="reference body"):
...
def test_stranger_attachments_stripped_not_persisted(mock_app, kind):
    ...
    expected = expected_stranger_inbound_outcome(
        block_all=False,
    ...
    assert row["content"] == "reference body"
...
def test_block_all_drops_stranger_attachment_message(mock_app):
    app = prepare_messaging_app(mock_app)
...
def test_contact_keeps_attachment_bytes(mock_app):
    app = prepare_messaging_app(mock_app)
    app.config.block_attachments_from_strangers.set(True)
    app.config.block_all_from_strangers.set(False)
...
def test_blocked_peer_attachment_message_dropped(mock_app):
    app = prepare_messaging_app(mock_app)
...
def test_stranger_text_without_attachments_still_stored(mock_app):
    app = prepare_messaging_app(mock_app)
    app.config.block_attachments_from_strangers.set(True)
    app.config.block_all_from_strangers.set(False)
