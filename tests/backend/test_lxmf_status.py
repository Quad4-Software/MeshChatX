...
"""Reference tests for LXMF state and method reporting.
...
def test_state_constant_maps_to_api_string(lxmf_state, expected):
...
def test_method_constant_maps_to_api_string(lxmf_method, expected):
...
def test_known_states_never_unknown(state):
...
def test_known_methods_never_unknown(method):
...
def test_unknown_state_ints_report_unknown(state):
...
def test_unknown_method_ints_report_unknown(method):
...
@settings(max_examples=64, deadline=None)
def test_message_dict_reports_lxmf_status_pair(state, method):
...
def test_propagated_sent_is_on_node_not_delivered():
...
def test_propagated_delivered_means_recipient_got_mail():
    payload = convert_lxmf_message_to_dict(
        _mock_lxmessage(
...
def test_direct_delivered_is_not_propagated():
    payload = convert_lxmf_message_to_dict(
...
def test_opportunistic_failed_still_reports_failed_state():
...
def test_paper_method_preserved_across_states():
