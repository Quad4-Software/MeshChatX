...
"""Independent references for LXMF sieve normalize and first-match."""
...
def test_caps_are_the_published_limits():
    caps = sieve_caps()
...
def test_normalize_folder_requires_int_folder_id():
    assert (
        normalize_lxmf_sieve_filters(
...
def test_json_roundtrip_of_normalized_rules():
    raw = [
        {
            "action": "folder",
            "terms": ["work"],
...
def test_scope_matrix(scope, is_contact, expect_match):
...
def test_first_enabled_rule_wins():
    rules = normalize_lxmf_sieve_filters(
        [
...
def test_regex_is_case_insensitive_dotall():
    rules = normalize_lxmf_sieve_filters(
...
def test_comma_separated_terms_string():
    rules = normalize_lxmf_sieve_filters(
...
def test_first_match_matches_independent_model(
    rules_raw,
    peer,
    is_contact,
    msg,
    ...
    expected = expected_sieve_first_match(
        rules,
        peer,
        is_contact=is_contact,
        message_haystack=msg,
...
def test_parse_never_raises(blob):
