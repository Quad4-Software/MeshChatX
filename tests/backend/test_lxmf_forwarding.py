...
"""References for LXMF forwarding rules, mappings, WS mutators, and handle_forwarding."""
...
def test_reply_path_beats_rules(mock_app):
    ...
    expected = expected_forward_sends(
...
def test_source_filter_and_inactive_rules(mock_app):
    app = prepare_messaging_app(mock_app)
...
def test_each_matching_rule_sends_once(mock_app):
    app = prepare_messaging_app(mock_app)
    app.current_context.forwarding_manager = MagicMock()
    app.current_context.forwarding_manager.get_or_create_mapping.return_value = {
        "alias_hash": PEER_ALIAS,
...
def test_inbound_delivery_triggers_forward(mock_app):
    app = prepare_messaging_app(mock_app)
    app.current_context.forwarding_manager = MagicMock()
    app.current_context.forwarding_manager.get_or_create_mapping.return_value = {
        "alias_hash": PEER_ALIAS,
...
def test_ws_add_get_toggle_delete(mock_app):
    app = prepare_messaging_app(mock_app)
    client = _ws_client()
    asyncio.run(
        handle_lxmf_forwarding_rule_add(
...
def test_ws_add_rejects_invalid_hashes(mock_app):
    app = prepare_messaging_app(mock_app)
    client = _ws_client()
    asyncio.run(
        handle_lxmf_forwarding_rule_add(
...
def test_ws_add_canonicalizes_hex(mock_app):
    app = prepare_messaging_app(mock_app)
...
def test_ws_add_without_forward_to_hash_is_noop(mock_app):
    app = prepare_messaging_app(mock_app)
    asyncio.run(
...
def test_mapping_roundtrip_sqlite(mock_app):
    app = prepare_messaging_app(mock_app)
    data = {
...
def test_get_or_create_mapping_reuses_row(tmp_path, db):
...
def test_null_identity_rule_forwards_any_source(mock_app):
    app = prepare_messaging_app(mock_app)
    app.current_context.forwarding_manager = MagicMock()
    app.current_context.forwarding_manager.get_or_create_mapping.return_value = {
        "alias_hash": PEER_ALIAS,
...
@pytest.mark.asyncio
async def test_ws_forwarding_add_requires_auth(mock_app):
