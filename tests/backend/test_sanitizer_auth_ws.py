...
"""References for HTML sanitization, auth verification, command environments, and privacy.
...
def test_safe_href_rejects_executable_schemes():
...
def test_markdown_render_never_emits_executable_href():
    for payload in _XSS_HREF_PAYLOADS:
...
def test_markdown_img_src_rejects_svg_and_html_data():
...
def test_kml_description_child_html_is_flattened():
...
def test_geojson_strips_data_html_and_vbscript_icon_keys():
    payload = json.dumps(
        {
...
def test_image_magic_rejects_html_svg_and_empty():
...
def test_executable_page_env_cannot_clobber_path_or_inject_loader():
...
def test_plugin_asset_path_rejects_traversal():
    for bad in (
        "../etc/passwd",
        "/etc/passwd",
        "foo/../../etc/passwd",
...
def test_ws_handlers_are_classified():
    from meshchatx.src.backend.websocket_config_guard import (
...
def test_config_dict_omits_secrets(mock_app):
...
def test_startup_status_omits_identity_material(mock_app):
...
def test_csrf_exempt_is_exact_path_only():
...
def test_parse_stored_lxmf_fields_fail_closed():
...
def test_redact_diagnostic_strips_keys_paths_hashes():
    raw = (
...
@pytest.mark.asyncio
async def test_auth_middleware_status_prefix_is_not_public():
    app = MagicMock()
...
async def test_protected_routes_401_when_auth_enabled(mock_app):
...
@pytest.mark.asyncio
@pytest.mark.usefixtures("require_loopback_tcp")
async def test_auth_setup_rejected_once_password_exists(mock_app):
    _enable_password_auth(mock_app)
    aio_app = _make_aio_app(mock_app)
    async with TestClient(TestServer(aio_app)) as client:
...
def test_safe_href_never_returns_executable_scheme(raw):
    out = _safe_href(raw)
