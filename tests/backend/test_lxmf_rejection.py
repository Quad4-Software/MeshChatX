...
"""References for LXMF rejection paths: spam, stranger attachments, telemetry bypass.

Independent model (expected_reject_inbound_outcome):
...
def expected_reject_inbound_outcome(
    *,
    block_all: bool,
...
def _deliver(app, source, fields, content="reference body", title=""):
...
def test_spam_plus_attachment_drops_entire_message(mock_app):
    ...
    expected = expected_reject_inbound_outcome(
        block_all=False,
...
def test_telemetry_request_cannot_bypass_stranger_attachment_strip(mock_app):
...
def test_telemetry_request_cannot_bypass_spam_attachment_drop(mock_app):
    app = prepare_messaging_app(mock_app)
...
def test_embedded_lxms_bytes_are_not_sideband_commands():
...
def test_blocked_peer_never_stores(mock_app):
    app = prepare_messaging_app(mock_app)
