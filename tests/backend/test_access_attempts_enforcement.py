...
def _make_aio_app(mock_app, use_https: bool):
    ...
    bad_mw, sqlite_mw, auth_mw, mime_mw, _cache_mw, sec_mw, csrf_mw, ip_mw, _demo_mw = (
        mock_app._define_routes(routes)
    )
    aio_app = web.Application()
    setup_session(aio_app, mock_app._encrypted_cookie_storage(use_https))
    aio_app.middlewares.extend(
        [bad_mw, sqlite_mw, auth_mw, mime_mw, _cache_mw, sec_mw, csrf_mw, ip_mw]
    )
