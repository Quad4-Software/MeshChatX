...
"""Property and adversarial references for plugin permission and URL helpers."""
...
def _is_external_expected(value):
...
class TestExternalHttpUrl:
    ...
    def test_is_external_http_url_matches_reference(self, value):
        expected = _is_external_expected(value)
        assert _is_external_http_url(value) is expected

    @given(value=st.text())
    @settings(max_examples=50, deadline=None)
    def test_is_http_url_matches_prefix(self, value):
...
class TestHostRoot:
    ...
    def test_host_root_derives_real_host(self, url):
        root = _host_root(url)
        if root is None:
            # Determine whether the reference reference also rejects this URL.
            try:
                parsed = urlparse(url)
            except (ValueError, UnicodeError):
                return
