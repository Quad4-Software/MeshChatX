...
"""Call timeout and web audio PCM references.

References are intentional invariants:
...
def test_call_timeout_always_in_bounds(raw):
    """Reference: every parseable/unparseable input clamps into the safe window."""
    out = clamp_call_timeout_seconds(raw)
    assert MIN_CALL_TIMEOUT_SECONDS <= out <= MAX_CALL_TIMEOUT_SECONDS


def test_call_timeout_known_cases():
...
def test_web_audio_drops_oversized_pcm(extra):
    """Reference: frames above MAX_PCM_BYTES never reach the sink."""
...
def test_web_audio_drops_exact_oversize_boundary():
    sink = _Sink()
    src = WebAudioSource(target_frame_ms=60, sink=sink)
...
# ---------------------------------------------------------------------------
# Light references: attachment-like field shapes (mirror serve guards)
# ---------------------------------------------------------------------------
...
def test_image_type_always_allowlisted(image_type):
    """Reference: served image types collapse to the raster allowlist."""
    out = _safe_image_type(image_type)
    assert out in {"png", "jpeg", "jpg", "gif", "webp", "bmp"}


def test_known_bad_attachment_shapes_rejected():
...
# ---------------------------------------------------------------------------
# References: bridge push with real source
# ---------------------------------------------------------------------------
...
def test_bridge_push_with_real_source(pcm):
