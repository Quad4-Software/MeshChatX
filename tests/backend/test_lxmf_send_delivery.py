...
"""References for LXMF outbound send, delivery proofs, and auto-resend cascades.
...
@pytest.fixture
def db(tmp_path):
    path = str(tmp_path / "send_delivery_expected.db")
...
def test_lxmf_sent_is_not_delivered_without_proof():
...
def test_progress_terminal_matrix(state, method, try_prop, expected):
...
def test_sent_without_propagated_never_terminal(method):
...
@pytest.mark.asyncio
async def test_progress_loop_keeps_polling_opportunistic_sent(db):
...
def test_claim_refuses_non_failed_states(db):
    peer = "d" * 32
    now = time.time()
...
def test_failed_only_lineage_does_not_count_as_recent_success(db):
...
def test_in_flight_sent_blocks_same_content_resend(db):
...
@pytest.mark.asyncio
async def test_auto_resend_skips_sent_rows_and_only_claims_failed(db):
    peer = "7" * 32
...
@pytest.mark.asyncio
async def test_auto_resend_emits_new_hash_not_same_hash(db):
    peer = "b" * 32
    old_hash = "c" * 32
...
@pytest.mark.asyncio
async def test_concurrent_resend_claims_send_once(db):
    peer = "e" * 32
    old_hash = "f" * 32
...
def test_no_receipt_cascade_contract_documented():
...
def test_lxmf_send_delivery_proved():
    pairs = 0
