...
"""References for paper-message and meshchatx URI ingest over the LXMF WS handlers."""
...
def test_empty_uri_is_noop(mock_app):
...
def test_map_link_accepts_numeric_lat_lon(mock_app):
    app = prepare_messaging_app(mock_app)
    client = _client()
    asyncio.run(
        handle_lxm_ingest_uri(
...
def test_map_link_rejects_non_numeric_coords(mock_app):
    app = prepare_messaging_app(mock_app)
    client = _client()
    asyncio.run(
        handle_lxm_ingest_uri(
...
def test_map_zoom_clamped(mock_app, zoom_raw, expected):
    app = prepare_messaging_app(mock_app)
    client = _client()
    asyncio.run(
        handle_lxm_ingest_uri(
...
def test_docs_link_and_unknown_host(mock_app):
    app = prepare_messaging_app(mock_app)
    client = _client()
    asyncio.run(
        handle_lxm_ingest_uri(
...
def test_lxm_uri_ingest_status_matrix(mock_app, router_result, status):
    app = prepare_messaging_app(mock_app)
