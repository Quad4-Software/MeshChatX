...
"""Security references for storage path jails, plugin isolation, and RRC membership."""
...
def test_rncp_send_refuses_ssl_and_database(tmp_path):
...
def test_rncp_fetch_save_refuses_ssl_and_storage_root(tmp_path):
...
def test_docs_zip_absolute_and_dotdot_leave_bait(tmp_path):
...
def test_dispatch_hook_skips_when_plugins_disabled(tmp_path):
...
def test_python_runtime_purges_nested_pycache(tmp_path):
...
def test_repository_upload_refuses_symlink_dest(tmp_path):
...
def test_kline_error_without_room_drops_all_client_rooms(tmp_path):
...
def test_kline_teardown_fans_parted_to_remaining_members():
    """Kline teardown must notify remaining members the same way kick does."""
    server = RRCHubServer(
        FakeRrcManager(), FakeIdentity(HUB_HASH), name="Reference Hub"
    )
    sent = []
...
def test_filesync_refuses_page_nodes_sync_root(tmp_path):
...
def test_rncp_shared_root_refuses_session_secret_and_sibling_identity(
    tmp_path,
):
...
def test_plugin_install_does_not_copy_symlink_target(tmp_path):
...
def test_rncp_refuses_page_nodes_send_and_fetch_save(tmp_path):
