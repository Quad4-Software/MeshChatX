...
"""Reference tests for identity hotswap serialization, delete eviction, and LXMF GET consistency."""
...
@pytest.mark.asyncio
async def test_concurrent_hotswap_serializes_critical_section(
    mock_rns,
    temp_dir,
):
...
@pytest.mark.asyncio
async def test_hotswap_broadcast_requires_reauth_when_auth_enabled(
    mock_rns,
    temp_dir,
):
    app = ReticulumMeshChat(
...
@pytest.mark.asyncio
async def test_hotswap_broadcast_no_reauth_when_auth_disabled(
    mock_rns,
    temp_dir,
):
    app = ReticulumMeshChat(
...
@pytest.mark.asyncio
async def test_propagation_nodes_reads_only_ctx_database(mock_rns, temp_dir):
    app = ReticulumMeshChat(
        identity=mock_rns["id_instance"],
        storage_dir=temp_dir,
        reticulum_config_dir=temp_dir,
    ...
    ctx.running = True
    marker = "reference-marker-identity"
