...
/**
 * Reference tests for unread badge / DND / identity-switch leakage across
...
describe("conversations unread reference", () => {
    beforeEach(() => {
    ...
    it("does not decrement nav unread when dismissing an already-read conversation", async () => {
        // Reference: badge count may only drop when the conversation was unread.
    ...
    it("marks a restored open pane read after leaving Messages and returning", async () => {
        // Reference: returning to Messages with the same pane still open must clear
...
describe("notifications DND unread reference", () => {
    it("still refreshes unread badge under DND (DND suppresses OS/sound only)", async () => {
        // Reference: DND must not freeze the Messages nav badge while other pages are open.
...
describe("map local URL reference", () => {
    it("treats only RFC1918 and loopback hosts as local, not all 172.*", () => {
        // Reference: 172.16.0.0/12 is private. 172.15.x and 172.32.x are not.
