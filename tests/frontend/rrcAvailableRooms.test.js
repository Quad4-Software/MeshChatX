...
/**
 * Reference tests for RRC available-rooms refresh sync.
 *
...
/** Independent reference for unjoined sidebar rows. */
function expectedUnjoined(availableRooms, knownRooms, keyedRooms) {
...
/** Independent reference for add/remove/topic-update after a list replace. */
function expectedDiff(previous, next) {
...
describe("rrcAvailableRooms references", () => {
    it("reference: unjoined rooms exclude known rooms and sort by name", () => {
        const available = { zebra: null, lobby: "Main", alpha: "A" };
        const known = ["lobby"];
        expect(unjoinedAvailableRooms(available, known)).toEqual(expectedUnjoined(available, known));
    ...
    it("reference: keyed listed rooms surface has_key without changing join order", () => {
        const available = { zebra: null, lobby: "Main", vault: "ops" };
        const known = ["lobby"];
        const keyed = ["vault", "missing"];
        expect(unjoinedAvailableRooms(available, known, keyed)).toEqual(expectedUnjoined(available, known, keyed));
    ...
    it("reference: empty or invalid available maps yield no unjoined rooms", () => {
    ...
    it("reference: list refresh replace adds new rooms and removes gone ones", () => {
        ...
        expect(diffAvailableRooms(previous, applied)).toEqual(expectedDiff(previous, applied));
    ...
    it("reference: empty list snapshot clears all available rooms", () => {
    ...
    it("fuzz: random maps match independent unjoined and diff references", () => {
        ...
        for (let i = 0; i < 80; i++) {
            ...
            expect(diffAvailableRooms(available, applied)).toEqual(expectedDiff(available, applied));
