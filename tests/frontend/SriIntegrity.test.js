...
describe("SRI (Subresource Integrity) Verification", () => {
    ...
    describe("geo-wasm", () => {
        it("has valid integrity.json with matching SRI hashes when built", () => {
            const integrityPath = path.join(REPO_ROOT, "meshchatx/src/frontend/public/vendor/geo-wasm/integrity.json");
            const wasmPath = path.join(REPO_ROOT, "meshchatx/src/frontend/public/vendor/geo-wasm/geo.wasm");
            const execPath = path.join(REPO_ROOT, "meshchatx/src/frontend/public/vendor/geo-wasm/wasm_exec.js");
