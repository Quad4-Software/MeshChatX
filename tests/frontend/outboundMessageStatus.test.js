...
/**
 * Reference tests for outbound LXMF status icons and titles.
 *
...
/**
 * Independent UI reference. Must stay in sync with product intent, not by
 * importing the implementation under test into the expected table.
 */
function expectedIconName({ state, method }) {
    if (state === "delivered") {
...
function expectedTitleKey({ state, method }) {
    if (state === "delivered") {
...
describe("outboundMessageStatus LXMF reference", () => {
    it("maps every API state x method pair to the independent icon reference", () => {
        for (const state of API_STATES) {
            for (const method of API_METHODS) {
                const msg = { state, method };
                expect(outboundBubbleStatusIconName(msg)).toBe(expectedIconName(msg));
            }
        }
    });

    it("maps every API state x method pair to the independent title-key reference", () => {
        for (const state of API_STATES) {
            for (const method of API_METHODS) {
                const msg = { state, method };
                expect(outboundBubbleStatusTitleKey(msg)).toBe(expectedTitleKey(msg));
            }
        }
    });
    ...
    it("treats only opportunistic+failed as deferred wait", () => {
        for (const state of API_STATES) {
            for (const method of API_METHODS) {
                const msg = { state, method };
                expect(isOpportunisticDeferredDelivery(msg)).toBe(expectedDeferred(msg));
            }
        }
    });
