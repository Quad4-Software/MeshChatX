...
/**
 * Runtime strategy references: races, leaks, update prune, navigation timeout fallback.
 */
...
function assertNoApiLeak(cacheStorage) {
    const leaked = findForbiddenCachedUrls(cacheStorage.allUrls());
    expect(leaked).toEqual([]);
}

describe("swShellRuntime reference / race / leak", () => {
    afterEach(() => {
        vi.useRealTimers();
    });

    it("reference: cache-first serves cached asset and never stores API", async () => {
        ...
        expect(expectedStrategy({ pathname: "/api/v1/status", method: "GET" })).toBe("bypass");
        assertNoApiLeak(caches);
    });

    it("reference: failed network responses are not written to cache", async () => {
        const caches = new MemoryCacheStorage();
...
describe("swClientRegister update lifecycle reference", () => {
    it("reference: first install does not reload, update reloads once", () => {
    ...
    it("reference: certificate errors are ignorable, others are not", () => {
    ...
    it("reference: register options force updateViaCache none", () => {
        expect(serviceWorkerRegisterOptions()).toEqual({ updateViaCache: "none" });
    });

    it("reference: service worker registers only outside Vite DEV and Electron", () => {
        const cases = [
...
describe("swShellRuntime navigation preload", () => {
    ...
    it("reference: attach preload only for same-origin navigations that resolveFetch handles", () => {
        const cases = [
        ...
        for (const input of cases) {
            ...
            const expected = expectedStrategy(input) === "navigation";
