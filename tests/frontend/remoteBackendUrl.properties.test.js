...
// Independent reference: only http(s) with a hostname, no userinfo, no fragment,
...
describe("remoteBackendUrl references", () => {
    it.each(cases)("normalizes %j according to the URL reference", (input) => {
        expect(normalizeRemoteBackendUrl(input)).toBe(reference(input));
    });
});
