...
describe("CJK Text Overflow UI CSS Tests", () => {
    it("verifies tailwind wrap-break-word classes prevent horizontal overflow for long CJK text", () => {
        const longChineseText =
            "这是一个非常长的中文字符串用于测试在没有空格的情况下是否会溢出容器边界如果它不换行就会破坏UI所以我们需要确保包含换行相关的CSS类比如wrap-break-word或者break-all这对于亚洲语言支持是非常重要的一环避免影响用户体验。";
        ...
        const MockBubble = {
            template: `
                <div class="message-bubble w-64 border border-red-500 overflow-hidden">
                    <div class="content leading-relaxed wrap-break-word [word-break:break-word] min-w-0 markdown-content">
                        {{ text }}
                    </div>
                </div>
            `,
